...
test("settings uses account phone and tools tabs with nested sections", async () => {
  ...
  assert.match(main, /const baseUrl = agentControlPlaneUrl\(\)/);
  assert.match(main, /if \(!baseUrl\) throw new Error\(unconfiguredAgentControlPlaneMessage\(\)\)/);
  ...
  assert.match(api, /previewAgentControlPlaneMessage/);
  assert.doesNotMatch(api, /agent-control-plane\.query\.prod\.telnyx\.io/);
